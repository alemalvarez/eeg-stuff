...
from test_package import DEFAULT_BAND
from tsallis_entropy import calcular_te as tsallis_entropy_psd
from shannon_entropy import calcular_se as shannon_entropy_psd
from renyi_entropy import calcular_re as renyi_entropy_psd
...
from eeg_utils import get_spectral_density as compute_psd
...
class TestTsallisEntropyPSD:
    ...
    def shannon_entropy_reference(self, psd):
        """Reference Shannon entropy implementation for comparison"""
    ...
    def test_convergence_to_shannon(self):
        ...
        for signal_name, sig in signals.items():
            ...
            f, psd = compute_psd(signal, CFG)
            psd = psd[0, :]
            shannon_entropy = self.shannon_entropy_reference(psd)
    ...
    def test_concavity_convexity_properties(self):
        ...
        q_concave = [1.2, 1.5, 2.0]  # Should be concave (q > 1)
        q_convex = [0.5, 0.8]        # Should be convex (0 < q < 1)
        
        for q in q_concave:
            entropy1 = tsallis_entropy_psd(psd1, f1, DEFAULT_BAND, q)
            entropy2 = tsallis_entropy_psd(psd2, f2, DEFAULT_BAND, q)
            f_combined = np.linspace(0, 1, len(mixed_psd))
            entropy_mixed = tsallis_entropy_psd(mixed_psd, f_combined, DEFAULT_BAND, q)
            
            expected_linear = lambda_val * entropy1 + (1 - lambda_val) * entropy2
            
            # For q > 1, should be concave: S(λp1 + (1-λ)p2) ≥ λS(p1) + (1-λ)S(p2)
        ...
        for q in q_convex:
            ...
            # For 0 < q < 1, should be convex: S(λp1 + (1-λ)p2) ≤ λS(p1) + (1-λ)S(p2)
            assert entropy_mixed <= expected_linear + self.tolerance, \
                f"Convexity violated for q={q}: {entropy_mixed} > {expected_linear}"
    ...
    def test_q_parameter_validation(self):
        ...
        try:
            ...
            # If q=1 is handled, should give Shannon entropy
            shannon_ref = self.shannon_entropy_reference(psd)
